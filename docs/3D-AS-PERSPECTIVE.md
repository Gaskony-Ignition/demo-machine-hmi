...
driven by tags", what closes that gap, and the three ways to close the rest.*
...
## Why not a third-party 3D module

The one evaluated for this demo does not run on Ignition 8.3 at all — it
declares an 8.1 requirement and the ModuleManager refuses to install it. Even
on 8.1 it could not animate this cell: it is a BIM/AEC model viewer, and its
scripting surface has no property, method or event that sets an entity's
position or rotation — only appearance and camera. A shoulder and an elbow are
rotations, so a module that can colour a part but not move it answers a
different question than this demo asks. The WebDev + three.js page stays
because it is the one thing that actually moves, it costs nothing, needs no
licence or model-conversion toolchain, and already runs air-gapped with the
library vendored.
